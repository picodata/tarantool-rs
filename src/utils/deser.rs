...
/// # Errors
///
/// Returns an error if `value` is not a `MessagePack` map.
...
pub(crate) fn find_and_take_single_key_in_map(key: u8, map: Vec<(Value, Value)>) -> Option<Value> {
    for (k, v) in map {
        if matches!(k, Value::Integer(x) if x.as_u64() == Some(u64::from(key))) {
            return Some(v);
        }
    }
    None
...
/// # Errors
///
/// Returns an error if `value` does not contain `IPROTO_DATA`.
...
/// # Errors
///
/// Returns an error if `value` does not contain `IPROTO_DATA` or its
/// payload cannot be deserialized into the requested type.
